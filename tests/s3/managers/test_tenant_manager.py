...
from s3.managers.tenant_manager import (
    SYSTEM_GROUPS,
    TenantManager,
    _is_tenant_group,
)
...
def mock_group_manager():
    ...
    gm.list_groups.return_value = ["t1", "t1ro", "t2", "globalusers"]
    gm.get_group_members.return_value = ["alice", "bob"]
    gm.group_exists.return_value = True
    gm.is_user_in_group.return_value = True
    return gm
...
class TestListTenants:
    ...
    async def test_is_member_flag(self, manager, mock_group_manager):
        mock_group_manager.list_groups.return_value = ["t1"]
    ...
    async def test_includes_ro_members_in_count(self, manager, mock_group_manager):
        mock_group_manager.list_groups.return_value = ["t1", "t1ro"]
