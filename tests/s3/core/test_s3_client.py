...
S3Client provides async wrapper around aiobotocore for S3/MinIO operations.
...
# =============================================================================
# TestS3ClientInitialization - Client setup and lifecycle
# =============================================================================


class TestS3ClientInitialization:
    """Tests for S3Client initialization and session management."""

    def test_init_with_config(self, mock_s3_config):
        """Test basic initialization with config."""
        client = S3Client(mock_s3_config)
    ...
    async def test_create_initializes_session(
        ...
        client = await S3Client.create(mock_s3_config)
    ...
    async def test_async_context_manager(
        ...
        """Test using S3Client as async context manager."""
        client = S3Client(mock_s3_config)
        assert client._session is None
...
class TestConnectionTesting:
    ...
    async def test_test_connection_success(
        ...
        mock_aiobotocore_client.list_buckets = AsyncMock(
            return_value={"Buckets": [{"Name": "bucket1"}, {"Name": "bucket2"}]}
        )

        async with mock_s3_client:
            result = await mock_s3_client.test_connection()
    ...
    async def test_test_connection_empty_buckets(
        ...
        mock_aiobotocore_client.list_buckets = AsyncMock(return_value={"Buckets": []})
...
class TestBucketOperations:
    ...
    @pytest.mark.asyncio
    async def test_create_bucket(self, mock_s3_client, mock_aiobotocore_client):
        ...
        mock_aiobotocore_client.create_bucket.assert_called_once_with(
            Bucket="test-bucket"
        )

    @pytest.mark.asyncio
    async def test_create_bucket_error(self, mock_s3_client, mock_aiobotocore_client):
    ...
    async def test_bucket_exists_true(self, mock_s3_client, mock_aiobotocore_client):
        ...
        async with mock_s3_client:
            result = await mock_s3_client.bucket_exists("test-bucket")

        assert result is True
        mock_aiobotocore_client.head_bucket.assert_called_once_with(
            Bucket="test-bucket"
        )

    @pytest.mark.asyncio
    async def test_bucket_exists_false(self, mock_s3_client, mock_aiobotocore_client):
        ...
        async with mock_s3_client:
            result = await mock_s3_client.bucket_exists("nonexistent-bucket")

        assert result is False

    @pytest.mark.asyncio
    async def test_bucket_exists_error(self, mock_s3_client, mock_aiobotocore_client):
        ...
        async with mock_s3_client:
            with pytest.raises(BucketOperationError, match="Bucket check failed"):
                await mock_s3_client.bucket_exists("forbidden-bucket")
    ...
    async def test_bucket_exists_unexpected_error(
        ...
        async with mock_s3_client:
            with pytest.raises(BucketOperationError, match="Bucket check failed"):
                await mock_s3_client.bucket_exists("test-bucket")

    @pytest.mark.asyncio
    async def test_list_buckets(self, mock_s3_client, mock_aiobotocore_client):
        ...
        async with mock_s3_client:
            buckets = await mock_s3_client.list_buckets()

        assert buckets == ["bucket1", "bucket2", "bucket3"]

    @pytest.mark.asyncio
    async def test_list_buckets_empty(self, mock_s3_client, mock_aiobotocore_client):
    ...
    @pytest.mark.asyncio
    async def test_list_buckets_error(self, mock_s3_client, mock_aiobotocore_client):
        ...
        async with mock_s3_client:
            with pytest.raises(BucketOperationError, match="Bucket listing failed"):
                await mock_s3_client.list_buckets()

    @pytest.mark.asyncio
    async def test_delete_bucket_empty(self, mock_s3_client, mock_aiobotocore_client):
        ...
        mock_aiobotocore_client.get_paginator = MagicMock(return_value=mock_paginator)
        mock_aiobotocore_client.delete_bucket = AsyncMock()

        async with mock_s3_client:
            await mock_s3_client.delete_bucket("test-bucket")

        mock_aiobotocore_client.delete_bucket.assert_called_once_with(
            Bucket="test-bucket"
        )
    ...
    async def test_delete_bucket_with_objects(
        ...
        mock_paginator.paginate = MagicMock(return_value=async_iterator)
        mock_aiobotocore_client.get_paginator = MagicMock(return_value=mock_paginator)
        mock_aiobotocore_client.delete_objects = AsyncMock()
        mock_aiobotocore_client.delete_bucket = AsyncMock()
        ...
        assert mock_aiobotocore_client.delete_objects.call_count == 2
        mock_aiobotocore_client.delete_bucket.assert_called_once_with(
            Bucket="test-bucket"
        )

    @pytest.mark.asyncio
    async def test_delete_bucket_error(self, mock_s3_client, mock_aiobotocore_client):
...
class TestObjectOperations:
    ...
    @pytest.mark.asyncio
    async def test_put_object(self, mock_s3_client, mock_aiobotocore_client):
        """Test putting an object."""
        mock_aiobotocore_client.put_object = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_put_object_error(self, mock_s3_client, mock_aiobotocore_client):
    ...
    @pytest.mark.asyncio
    async def test_get_object(self, mock_s3_client, mock_aiobotocore_client):
        """Test getting an object."""
        mock_body = MagicMock()
        mock_body.read = AsyncMock(return_value=b"object content")
        mock_aiobotocore_client.get_object = AsyncMock(return_value={"Body": mock_body})

        async with mock_s3_client:
            content = await mock_s3_client.get_object("test-bucket", "path/to/object")

        assert content == b"object content"
        mock_aiobotocore_client.get_object.assert_called_once_with(
            Bucket="test-bucket", Key="path/to/object"
        )

    @pytest.mark.asyncio
    async def test_get_object_error(self, mock_s3_client, mock_aiobotocore_client):
        """Test getting an object with error."""
        mock_aiobotocore_client.get_object = AsyncMock(
            side_effect=Exception("Object not found")
        )

        async with mock_s3_client:
            with pytest.raises(BucketOperationError, match="Object get failed"):
                await mock_s3_client.get_object("test-bucket", "nonexistent")

    @pytest.mark.asyncio
    async def test_delete_object(self, mock_s3_client, mock_aiobotocore_client):
        """Test deleting an object."""
        mock_aiobotocore_client.delete_object = AsyncMock()

        async with mock_s3_client:
            await mock_s3_client.delete_object("test-bucket", "path/to/object")
    ...
    @pytest.mark.asyncio
    async def test_delete_object_error(self, mock_s3_client, mock_aiobotocore_client):
    ...
    @pytest.mark.asyncio
    async def test_list_objects_basic(self, mock_s3_client, mock_aiobotocore_client):
        ...
        async with mock_s3_client:
            objects = await mock_s3_client.list_objects("test-bucket")
    ...
    async def test_list_objects_with_prefix(
        ...
        async with mock_s3_client:
            objects = await mock_s3_client.list_objects("test-bucket", prefix="users/")
    ...
    @pytest.mark.asyncio
    async def test_list_objects_empty(self, mock_s3_client, mock_aiobotocore_client):
    ...
    @pytest.mark.asyncio
    async def test_list_objects_respects_limit(
        self, mock_s3_client, mock_aiobotocore_client
    ):
    ...
    async def test_list_objects_list_all_ignores_limit(
        ...
        async with mock_s3_client:
            objects = await mock_s3_client.list_objects("test-bucket", list_all=True)
    ...
    @pytest.mark.asyncio
    async def test_list_objects_error(self, mock_s3_client, mock_aiobotocore_client):
        """Test listing objects with error."""
        mock_aiobotocore_client.get_paginator = MagicMock(
            side_effect=Exception("Pagination failed")
        )

        async with mock_s3_client:
            with pytest.raises(BucketOperationError, match="Object listing failed"):
                await mock_s3_client.list_objects("test-bucket")
