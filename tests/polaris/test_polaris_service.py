...
class TestUpdateCatalogStorageConfig:
    """Tests for in-place catalog storage metadata updates."""

    @pytest.mark.asyncio
    async def test_update_catalog_storage_config_puts_expected_payload(
        self, polaris_service
    ):
        with patch.object(
            polaris_service, "_request", new_callable=AsyncMock
        ) as mock_req:
            mock_req.return_value = {"catalog": {"name": "user_alice"}}

            result = await polaris_service.update_catalog_storage_config(
                "user_alice",
                "s3://cdm-lake/users-sql-warehouse/alice/iceberg/",
                current_entity_version=42,
            )
            ...
            mock_req.assert_called_once_with(
                ...
                json={
                    ...
                    "currentEntityVersion": 42,
                },
            )
            assert result == {"catalog": {"name": "user_alice"}}

    @pytest.mark.asyncio
    async def test_update_catalog_storage_config_can_override_endpoint(
        self, polaris_service
    ):
        with patch.object(
            polaris_service, "_request", new_callable=AsyncMock
            ...
            await polaris_service.update_catalog_storage_config(
                "tenant_team1",
                "s3://cdm-lake/tenant-sql-warehouse/team1/iceberg/",
                s3_endpoint="https://s3.amazonaws.com",
            )

            payload = mock_req.call_args.kwargs["json"]
            assert "currentEntityVersion" not in payload
            storage_config = payload["storageConfigInfo"]
            assert storage_config["endpoint"] == "https://s3.amazonaws.com"
            assert storage_config["endpointInternal"] == "https://s3.amazonaws.com"
